...
public record CreatePaySlipByRangeDateCommand() : IRequest<string>
{
...
public class CreatePaySlipByRangeDateCommandHandler : IRequestHandler<CreatePaySlipByRangeDateCommand, string>
{
    private readonly IApplicationDbContext _context;

    public CreatePaySlipByRangeDateCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<string> Handle(CreatePaySlipByRangeDateCommand request, CancellationToken cancellationToken)
    {
        try
        {
            //khai báo
            ...
            //------Các lỗi chưa tính phiếu lương được-------
            //-----------------------------------------------
            #region Exceptions
            ...
            {
                throw new NotFoundException($"Không tìm thấy hợp đồng của nhân viên có Id: {request.EmployeeId}");
                //return "Không tìm thấy hợp đồng của nhân viên";
            }

            //request.StartDate = new DateTime(request.StartDate.Year, request.StartDate.Month, request.StartDate.Day, 0, 0, 0);
            var Last_PaySlip = await _context.PaySlips
                .Where(x => x.EmployeeContractId == EmployeeContract.Id)
                .SingleOrDefaultAsync(cancellationToken);
            if (Last_PaySlip == null)
            {
                if (EmployeeContract.StartDate < request.StartDate.Date)
                {
                    throw new NotFoundException("Chưa bắt đầu hợp đồng, chưa thể tính lương");
                }
            }
            else
            {
                if (Last_PaySlip.Paid_date >= request.StartDate.Date)
                {
                    throw new NotFoundException($"Hợp đồng này đã được tính lương gần đây, chưa thể tính lương tiếp tục");
                }
            }

            var OvertimeLogPedingList = await _context.OvertimeLogs
               .Where(x => x.EmployeeId == EmployeeContract.EmployeeId && x.Status == OvertimeLogStatus.Pending 
               && x.IsDeleted == false && x.StartDate.Date >= request.StartDate.Date && x.EndDate.Date <= request.EndDate.Date)
               .ToListAsync(cancellationToken);
            if (OvertimeLogPedingList.Count > 0)
            {
                throw new NotFoundException($"Hãy duyệt đầy đủ đơn Tăng Ca trước khi chấm lương");
            }
            var LeaveLogPedingList = await _context.LeaveLogs
                .Where(x => x.EmployeeId == EmployeeContract.EmployeeId && x.Status == LeaveLogStatus.Pending 
                && x.IsDeleted == false && x.StartDate.Date >= request.StartDate.Date && x.EndDate.Date <= request.EndDate.Date)
                .ToListAsync(cancellationToken);
            if (LeaveLogPedingList.Count > 0)
            {
                throw new NotFoundException($"Hãy duyệt đầy đủ đơn Xin Nghỉ trước khi chấm lương");
            }
            #endregion
            //------------------------------------------------
            //------------------------------------------------
            ...
            .Where(x => x.Day.Date >= request.StartDate.Date && x.Day.Date <= request.EndDate.Date && x.TypeDate == TypeDate.Weekday)
            .ToListAsync(cancellationToken);
            if (AnnualWorkingDays.Count == 0)
            {
                throw new NotFoundException("Danh sách danh sách ngày làm việc hằng năm trống. Vui lòng cập nhật danh sách");
            }
            Standard_Work_Hours = AnnualWorkingDays.Count * 8;
            #endregion
            //Lương tiêu chuẩn cho 1 giờ làm
            #region
            Salary_1Hour = EmployeeContract.Salary / Standard_Work_Hours;
            #endregion
            //lấy danh sách ngày mà nhân viên này có điểm danh (nếu bảng này không có OT thì phần dưới lọc weekday bị thừa) 
            #region
            var TimeAttendanceLogList = await _context.TimeAttendanceLogs
                .Where(x => x.EmployeeId == EmployeeContract.EmployeeId && x.StartTime.Date >= request.StartDate.Date && x.EndTime.Date <= request.EndDate.Date)
                .ToListAsync(cancellationToken);
            #endregion
            //Loại bỏ đi những ngày k phải Weekday và tính tổng số thời gian làm việc thực tế trong khoảng thời gian quy định
            #region
            var TimeAttendanceLog = new List<TimeAttendanceLog>();
            foreach (var day in TimeAttendanceLogList)
            {
                var dayChecked = _context.AnnualWorkingDays.FirstOrDefault(a => a.Day.Date == day.StartTime.Date);
                if (dayChecked is not null && dayChecked.TypeDate == TypeDate.Weekday)
                {
                    TimeAttendanceLog.Add(day);
                }
            }
 
            if (TimeAttendanceLog.Count == 0)
            {
                throw new NotFoundException("Không tìm thấy lịch sử chấm công của nhân viên");
            }
            foreach (var Log in TimeAttendanceLog)
            {
                if (Log.Ducation == 0)
                {
                    throw new NotFoundException("Vui lòng tính chấm công cho nhân viên đầy đủ trước khi tính lương.");
                }
                if (Log.Ducation > 8)
                {
                    Actual_Work_Hours += 8;
            ...
            #endregion
            //Tính số xin nghỉ thuộc diện vẫn dc nhận lương để cộng vào lương (cmt thieues noi dung)
            #region
            var LeaveLogApproved = await _context.LeaveLogs
                .Where(x => x.EmployeeId == EmployeeContract.EmployeeId && x.Status == LeaveLogStatus.Approved 
                        && x.IsDeleted == false && x.StartDate.Date >= request.StartDate.Date && x.EndDate.Date <= request.EndDate.Date
                        && x.LeaveType.IsReward == true
                      )
                .ToListAsync(cancellationToken);
            var Leave_Hours_Approved = LeaveLogApproved.Sum(x => x.LeaveHours);

            var OvertimeLog = await _context.OvertimeLogs
                .Where(x => x.EmployeeId == EmployeeContract.EmployeeId && x.Status == OvertimeLogStatus.Approved
                        && x.IsDeleted == false && x.StartDate.Date >= request.StartDate.Date && x.EndDate.Date <= request.EndDate.Date
                        )
                .ToListAsync(cancellationToken);
            
            var LeaveLog = await _context.LeaveLogs
                .Where(x => x.EmployeeId == EmployeeContract.EmployeeId
                        && x.IsDeleted == false && x.StartDate >= request.StartDate && x.EndDate <= request.EndDate
                        //&& x.LeaveType.IsReward == false
                      )
                .ToListAsync(cancellationToken);
            ...
            Salary = (Actual_Work_Hours * Salary_1Hour) 
                    + (Leave_Hours_Approved * Salary_1Hour)
                    + (Ot_Hours * Salary_1Hour); 

            foreach (var overtimeLog in OvertimeLog)
            {
                Bonus += (overtimeLog.TotalHours * (overtimeLog.Coefficients - 1) * Salary_1Hour);
            }
            #endregion
            //Tổng giờ nghỉ - tính luôn những hôm nghỉ Approved và IsReward
            #region
            Leave_Hours = LeaveLog.Sum(x => x.LeaveHours);
            Deduction = Leave_Hours * Salary_1Hour; //Tổng số lương không đi làm
            #endregion
            //Lấy List Income Tax và List Exchange
            #region
            ...
            //nếu là lương Gross
            #region
            if (EmployeeContract.SalaryType == SalaryType.Gross)
            {
                Gross = Salary;
                //tính các loại bảo hiểm nhân viên phải trả 
                //tính trên lương
                if (EmployeeContract.CustomSalary > 0 || EmployeeContract.CustomSalary == null)
                {
                    BHXH_Emp = Gross * 0.08;
                    if (BHXH_Emp > 2384000)
                    {
