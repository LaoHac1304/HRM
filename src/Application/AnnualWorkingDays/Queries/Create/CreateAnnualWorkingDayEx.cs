...
public class CreateAnnualWorkingDayExCommandHandler : IRequestHandler<CreateAnnualWorkingDayEx, string>
{
    ...
    {
        //sử dụng để đặt ngữ cảnh giấy phép sử dụng của gói ExcelPackage thành phi thương mại
        ...
        try
        {
            using (var package = new ExcelPackage(new FileInfo(request.FilePath)))
            {
                ...
                if (worksheet == null)
                {
                    throw new InvalidDataException("File Excel không hợp lệ.");
                }
                ...
                {
                    throw new NotFoundException("Không tìm thấy ngày làm việc hàng năm hợp lệ trong tệp Excel.");
                }
        ...
        {
            throw new Exception("Sai định dạng tệp Excel.", ex);
        }
        catch (IOException ex)
        {
            throw new Exception("Lỗi khi truy cập file Excel", ex);
        }
    }
    ...
    private bool IsHoliday(DateTime date)
    {
        ...
        {
            throw new Exception($"Lỗi trong IsHoliday: {ex}");
        }
    }
    ...
    private DateTime GetTetNguyenDanStart(int year)
    {
        ...
        {
            throw new Exception($"Lỗi trong GetTetNguyenDanStart: {ex}");
        }
    }
    ...
    private DateTime GetGioToHungVuong(int year)
    {
        ...
        {
            throw new Exception($"Lỗi trong GetGioToHungVuong {ex}");
        }
    }
    ...
    private bool IsWeekend(DateTime date)
    {
        ...
        {
            throw new Exception($"Lỗi trong IsWeekend {ex}");
        }
    }
}